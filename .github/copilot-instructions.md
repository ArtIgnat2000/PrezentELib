...
- **Runtime flow**: `index.htm` renders slides; `script.js` controls slide state via `currentSlide`, `showSlide(n)`, `nextSlide()`, `prevSlide()` and adds keyboard handlers. There is no backend or build step.
...
- **Navigation**: `script.js` binds to elements with IDs `prevBtn` and `nextBtn`. Do not rename these IDs unless you update the selectors and their event bindings.
- **Animations**: `showSlide(n)` toggles `active`, `next`, `prev` classes — follow this pattern for CSS transitions.
...
- Inline styles are used in several slide elements for layout/decoration; when changing styling prefer updating `styles.css` but preserve inline fallbacks if present.
